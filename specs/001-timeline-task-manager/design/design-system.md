...
| 3 | ✅ **Đã đóng.** Mọi gói có mã native phải nằm trong `dependencies`, kể cả khi npm đã tự kéo về dưới dạng peer: `@op-engineering/op-sqlite`, `@dr.pogodin/react-native-fs`, `react-native-worklets`, `react-native-nitro-modules`. `react-native-quick-crypto` **không cần** vì mã hóa tắt. | Autolinking dựng danh sách module native từ **dependencies đã khai báo**. Một gói chỉ tồn tại dưới dạng peer sẽ có mặt trong JavaScript và **vắng trong binary native** — app build được, chạy được, rồi chết ở lần gọi đầu tiên với `TurboModuleRegistry.getEnforcing(...) could not be found`. Ràng buộc này giờ do `npm run check:native` giữ, chạy tự động sau mỗi `npm install`. |
